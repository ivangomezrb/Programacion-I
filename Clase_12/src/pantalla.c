	...
	if(aArray != NULL && cantidad>0)
	{
		...
		for(i=0;i<cantidad;i++)
		{
			if(aArray[i].status == STATUS_NOT_EMPTY){
				printf("Id: %d - Nombre: %s - Direccion: %s - Precio: %f - Tipo: %d\n",aArray[i].idPantalla,aArray[i].nombre,aArray[i].direccion,aArray[i].precio,aArray[i].tipo);
			}
		}
	}
	return retorno;
...
int pedirDatosPantalla(struct sPantalla* pPantalla)
{
	...
	getFloat(&bPantalla.precio,"Ingrese precio: \n","Error",0,100000000000,2);
	getInt(&bPantalla.tipo,"Ingrese tipo 1/2:\n","Error",1,2,2);
	*pPantalla = bPantalla;
	return retorno;
}
...
{
	struct sPantalla bPantalla;
	strncpy(bPantalla.nombre,"pepe",40);
	strncpy(bPantalla.direccion,"hola",40);
	bPantalla.precio = 70.00;
	bPantalla.tipo = 1;
	*pPantalla = bPantalla;
}
